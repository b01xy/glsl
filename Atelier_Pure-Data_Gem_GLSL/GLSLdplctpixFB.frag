...
void main()
{
	vec4 colorBase = texture2D(Tex0, gl_TexCoord[0].st);
	vec4 colorDessus = texture2D(Tex0, gl_TexCoord[0].st+vec2(0.01,0.01));
	if (colorDessus.r > xdeform) colorBase = colorDessus;
